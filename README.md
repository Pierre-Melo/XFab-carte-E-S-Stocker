...
Matériel nécessaire :  
- Le fichier UF2 [*Nuke*](£ link nuke uf2) permettant d’effacer entièrement la mémoire flash de la carte de développement.
- Le fichier UF2 [*CircuitPython*](£ link circuitpython uf2) correspondant à la version de la carte de développement.
- Les fichiers [*code.py*](£link code.py) et [*repl.py*](£link repl.py) trouvables dans le dossier [code](£ link code) du projet.
...
![photo bouton bootsel](£ photo bouton bootsel)  
...
3. Copiez le fichier [*Nuke*](£ lien nuke) dans `RPI-RP2`. Le support se ferme automatiquement. 
Débranchez puis répétez la procédure, cette fois avec le fichier [*CircuitPython*](£ lien circuitpython).  
4. Rebranchez : vous devez maintenant voir apparaître `CIRCUITPY`. Si ce n’est pas le cas, recommencez.  
5. Copiez [*code.py*](£ lien code.py) et [*repl.py*] à la racine de `CIRCUITPY`. Acceptez l’écrasement des fichiers existants.  
...
Matériel nécessaire :  
- Le fichier UF2 [*CircuitPython*](§ link circuitpython uf2 custom), disponible dans le dossier [*circuitpython*](£ lien circuitpython dossier) du projet.
- Les fichiers [*code.py*](£link code.py) et [*repl.py*](£link repl.py) trouvables dans le dossier [code](£ link code) du projet.
...
![photo via bootsel](£ photo via bootsel)  
2. Si l’opération est réussie, un support nommé `RPI-RP2` apparaît dans l’explorateur de fichiers. 
Si vous ne voyez que `CIRCUITPY` ou rien du tout, recommencez.  
3. Copiez le fichier [*CircuitPython*](£ lien circuitpython) dans `RPI-RP2`. Le support se ferme automatiquement.  
4. Rebranchez : vous devez maintenant voir apparaître `CIRCUITPY`. Si ce n’est pas le cas, recommencez.  
5. Copiez [*code.py*](£ lien code.py) et [*repl.py*] à la racine de `CIRCUITPY`. Acceptez l’écrasement des fichiers existants.  
...
- [] Recompiler circuitpython avec le build custom
- [] Réparer le README.md (photos et liens)
